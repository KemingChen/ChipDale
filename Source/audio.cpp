...
void CAudio::PauseById(int id)
{
    ...
    //for (i = info.begin();j<id && i != info.end(); i++){j++;}
    //	if (i->second.isGood) {
...
void CAudio::PlayOnSpe(unsigned id, bool repeat_flag, int time)
{
    if (!isOpened)
        return;
    GAME_ASSERT(info.find(id) != info.end(), "Can not play back audio: incorrect Audio ID!");
    ...
    char command[MAX_MCI_COMMAND_SIZE];
    if (repeat_flag)
        sprintf(command, "play device%d from %d repeat", id, time);
    else
        sprintf(command, "play device%d from 0", id);
