...
{
#define UP_FLAG		flag[0]
#define DOWN_FLAG	flag[1]
#define LEFT_FLAG	flag[2]
#define RIGHT_FLAG	flag[3]
#define A_FLAG		flag[4]
#define B_FLAG		flag[5]
...
class BasicInstructions : public CFrame
{
    public:
        ...
        void Squat();
        int FallingDown(MapManage* map);
        virtual ChipDale* getPartner()
        {
            return NULL;
        }
        virtual int CollisionChipDale(int Direct, int passSpeed, int Squeezemode = 1)
        {
            return false;
        }
        virtual void TriggerObj(int Derict)
        {
            TRACE("TriggerObj error\n");   // Derict吃花的方向
        }
        virtual int SetNowTaken(int Value)
        {
            return -1;   //設定NowTaken
        }
        virtual ChipDale* Instance()
        {
            return NULL;
        }
    protected:
