...
class Monster : public AI_Instructions
{
    public:
        ...
        virtual bool OnMove(MapManage* map, ChipDale** player)
        {
            return false;
        }
        ...
        virtual bool KillMonster(int Direct)
        {
            return false;
        }
...
class bullet : public Monster
{
    public:
        ...
        int Collision(ChipDale* player)
        {
            return player->IfCollision(wx, wy, width, height);
        }
    protected:
        int MoveLR, MoveUD;
};
...
class Electric : public Monster
{
    public:
        ...
        void OnShow(MapManage* map);
        bool OnMove(MapManage* map, ChipDale** player);
        int  ElectricCollision(ChipDale* player)
        {
            return player->IfCollision(wx, wy, width, height);
        }
    private:
...
class Centipedelimbs : public Monster
{
    ...
    private:
        static CAnimation frame_monster[4];
...
class Centipede : public Monster
{
    public:
        ...
        void CollisionChipDale(ChipDale* player);
        void FixMapMove(int fixX, int fixY);
        int IfCollision(int twx, int twy, int twidth, int theight)
        {
            return real_monster[0]->IfCollision(twx, twy, twidth, theight);
        }
        bool KillMonster(int Direct);
    private:
