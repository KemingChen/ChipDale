...
{
    //*************************************************
    //	If_Reset_State 好像可以刪除!!!
    //	IsLessCollision 好像也可以刪除!!!
    //*************************************************
    int i = 0;
    int fixY = 0, fixX = 0;
    int IsLand = false;
    ...
    //初始化
    /////////////////////////
    //暫時性修正　在未修正前也能判斷正常  NoTempFixXY 可關閉此功能(預設false 就是要修正)

    //if(map->MoveMap(0)&1)fixY = Jump_Fix;
    //if(map->MoveMap(0)&4)fixX = SPEED;
    //if(map->MoveMap(0)&8)fixX = -SPEED;
    ...
    else if(UpSpeed > 0 && map->IfCollision(wx + fixX, wy + fixY - abs(UpSpeed), width, abs(UpSpeed), NoIgnore_2))
    {
        /*向上跳過程遇到障礙物*/
        i = 0;
        while(!map->IfCollision(wx + fixX, wy - i + fixY - 1, width, 1, NoIgnore_2))i++; // 碰撞的修正
        wy -= i;
        UpSpeed = 0;
    }
    else
...
bool BasicInstructions::MoveLeft(MapManage* map)
{
    ...
    else if(temp)
    {
        ...
        //FIRST 腳色完全貼近
        i = SPEED - 1;
        while(i >= 0 && CollisionChipDale(4, i, 0))i--;
        SetWxWy(-i, 0, true);
        //移動剩餘額度 SPEED-i
    ...
    else
    {
        //撞到地圖障礙物修正
        i = 0;
        while(!map->IfCollision(wx - i, wy, width, height, true))i++;
...
bool BasicInstructions::MoveRight(MapManage* map)
{
    ...
    else if(temp)
    {
        ...
        //FIRST 腳色完全貼近
        i = SPEED - 1;
        while(i >= 0 && CollisionChipDale(8, i, 0))i--;
        SetWxWy(i, 0, true);
        //移動剩餘額度 SPEED-i
...
int BasicInstructions::FallingDown(MapManage* map)
{
    ...
    /*else if(getPartner()->ReturnIsTaken()&&getPartner()->ReturnNowAct()==4&&!getPartner()->ReturnHideComplete()&&wy>(getPartner()->ReturnWY()+getPartner()->ReturnHeight()-34)){
    	wy=(getPartner()->ReturnWY()+getPartner()->ReturnHeight()-34);
        UpSpeed = 0;
    	time_jump = 0;
    	If_Reset_State = true;
    	IsJump = false;
    ...
    else if(UpSpeed > 0 && map->IfCollision(wx + tempFixX, wy + tempFixY - abs(UpSpeed), width, abs(UpSpeed), true))
    {
        /*向上跳過程遇到障礙物*/
        i = 0;
        while(!map->IfCollision(wx + tempFixX, wy - i + tempFixY - 1, width, 1, true))i++; // 碰撞的修正
        wy -= i;
        UpSpeed = 0;
        time_jump = TIMEFORJUMP;
    }
