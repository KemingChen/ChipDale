...
class ChipDale : public BasicInstructions
{
    public:
        ...
        int ReturnHideComplete()
        {
            return (animation[IsDale][1][4][LRflag].GetCurrentBitmapNumber() >= 4) && NowAct == 4;
        }
        int ReturnNowAct()
        {
            return NowAct;
        }
        bool ReturnIsHurting()
        {
            return Hurt;
        }
        int ReturnLastAct()
        ...
        int ReturnIsTaken()
        {
            if(NowTaken > 0)return 1;
            else return 0;
        }
        int ReturnTakenByPartner()
        {
            if(Partner != NULL && Partner->NowTaken == ChipDale_taken)return 1 ;
            else return 0;
        }
        int ReturnHealth()
        {
        ...
        void SetPartner(ChipDale* setPlayer)
        {
            Partner = setPlayer;
        }
        void SetInvincible(int value)
        {
            Invincible = value;   //設定Invincible 1無敵 0正常
        }
        ...
        void ResetScore()
        {
            Score_Flower = Score_Star = 0;
        }

        void Dead();
        void Lock(bool value = true )
        {
            LOCK = value;
        }
