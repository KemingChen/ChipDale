...
class CAnimation
{
    public:
        ...
        void  AddBitmap(char*, char*, char*, COLORREF = CLR_INVALID);
        // 增加一張圖形至動畫(圖的編號及透明色)
        ...
        void  OnMoveToNum(int num);		// ZhongTing 更換到指定的bitmap
        void  OnShow();					// 將動畫貼到螢幕
        void  Reset();					// 重設播放順序回到第一張圖形
        void  SetDelayCount(int);		// 設定動畫播放速度的常數(越大越慢)
        void  SetBottomLeft(int x, int y, int height);// 將動畫的左上角座標移至 (x,y)
