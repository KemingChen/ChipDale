...
ChipDale* Monster::Detect(ChipDale** player, int* WLength, int* HLength, int WRange, int HRange, bool IfTraceInvincible)
{
    ...
    for(int i = 0; i < 2; i++)
    {
        ...
        //TRACE("player: %d %d %d %d\n",player[i]->ReturnWX(),player[i]->ReturnWY(),player[i]->ReturnWidth(),player[i]->ReturnHeight());
        //TRACE("square: %d %d %d %d , Range: %d %d\n",twx,twy,twidth,theight,WRange,HRange);
    ...
    //if(result!=NULL)TRACE("Length: %d %d\n",*WLength,*HLength);
    return result;
}
...
void Electric::Loading()
{
    ...
    TRACE("Electric Loading!!!(Under Class Wire)\n");
}
...
{
    if(NowAct < 0)return false;
    //FixXY(map); //因為不是正常的怪物 所以她已經FIX過了
...
///////////////////////////////////////////////////////////
//Centipedelimbs
CAnimation Centipedelimbs::frame_monster[4];
Centipedelimbs::Centipedelimbs(MapManage* map, int SetWx, int SetWy, int SetSelect): ReleaseBullTime(15), TotalBullTime(50), BulletMaxSpeed(10), WRandBullet(100)
{
    ...
    wx = SetWx;
    wy = SetWy;
    TRACE("Centipedelimbs!!!  wx %d  , wy %d\n", wx, wy);
    NowAct = 0;
    Health = 1;
    CanTrace = false;
    Select = SetSelect;
...
void Centipedelimbs::Loading()
{
    ...
    frame_monster[3].SetDelayCount(2);
    frame_monster[3].AddBitmap("Bitmaps/action/Centipede/head_ntut2.bmp", PURPLE);
    frame_monster[3].AddBitmap("Bitmaps/action/Centipede/head_ntut_die2.bmp", PURPLE);
    TRACE("Centipedelimbs Loading!!!\n");
}
    ...
    if(wx > -WinShowBuffer && wx < MWIDTH && wy > -WinShowBuffer && wy < MHEIGHT)
    {
        switch(Select)
        {
            case 0:
                ...
                {
                    if(countFlicker == 0)
                        frame_monster[3].Reset();
                    frame_monster[3].SetBottomLeft(wx - 10, wy, height);
                    frame_monster[3].OnShow();
                    frame_monster[3].OnMove();
                }
                break;
            case 1:
            case 2:
