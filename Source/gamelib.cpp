...
void CAnimation::AddBitmap(char* path, char* name, char* file, COLORREF colorkey)
{
    CMovingBitmap add_bmp;
    char temp[100];
    sprintf(temp, "%s%s%s", path, name, file);
    add_bmp.LoadBitmap(temp, colorkey);
    bmp.insert(bmp.end(), add_bmp);
    Reset();
}
...
void CAnimation::OnMoveToNum(int num)
{
    GAME_ASSERT(bmp.size() != 0, "CAnimation: Bitmaps must be loaded first.");
    if (--delay_counter <= 0)
    {
...
void CAnimation::SetBottomLeft(int nx, int ny, int fix_y)
{
    x = nx, y = ny + fix_y;
...
void CAnimation::OnShow()
{
    ...
    bmp_iter->SetTopLeft(x, y - bmp_iter->Height());
    bmp_iter->ShowBitmap();
}
    ...
    : NUM_GAME_STATES(5)
{
    ...
    gameStateTable[GAME_STATE_WORLD] = new CGameStateWorlds(this);
    gameState = NULL;
}
